...
def plot_efficiency_vs_p(param, KD, Kp_pump, Kp_waste_vals, V_base, kappa, cDc, cpp_axis):
    ''' PLOT CHEMICAL EFFICIENCY AS A FUNCTION OF [p] FOR VARIOUS KD VALUES '''

    efficiency = len(Kp_waste_vals)*[[]] # Allocate array for chemical efficiency values

    # Evaluate efflux at each value of periplasmic proton concentration
    for i in range(len(Kp_waste_vals)):
        Kp_list = [Kp_pump, Kp_waste_vals[i]] # Cycle through different values of K_p for the waste cycle

        for j in range(len(cpp_axis)):
            cpp = cpp_axis[j]
            
            efflux = pump.efflux_numerical_4(param, KD, Kp_list, V_base, kappa, cDc, cpp)
            p_flux = pump.p_flux_4(param, KD, Kp_list, V_base, kappa, cDc, cpp)
            efficiency[i] = efficiency[i] + [efflux/p_flux] # Chemical efficiency

    # Configure some things for plotting
    ls_list = [(0,(1,1)), "dashdot", "dashed", (0,(3,1,1,1,1,1))] # Linestyle list, for plotting
    Kp_waste_vals_uM = microfy(Kp_waste_vals) # Kp_vals in uM
    cpp_axis_uM = microfy(cpp_axis) # cpp_axis in uM

    plt.figure()
    for i in range(len(Kp_waste_vals)):
        plt.semilogx(cpp_axis_uM, efficiency[i],label="$K_{p,waste} =\:"+str(int(Kp_waste_vals_uM[i]))+"\:\mu M$",linestyle=ls_list[i])
...
KD = 1e-6
Kp_pump = 1e-6 #M, proton binding affinities for pumping cycle
...
Kp_waste_vals = [1e-6, 5e-6, 1e-5, 5e-5]
cpp_axis = np.logspace(-8, -3)

#### MAIN CALLS ####

param = Params4(rD, rp, rt, cDo, cpc, vD, vp_list) # Create instantiation of Params4 object

# plot_efflux_vs_KD(param, KD_axis, Kp_list, V_base, kappa, cDc, cpp_vals)
plot_efficiency_vs_p(param, KD, Kp_pump, Kp_waste_vals, V_base, kappa, cDc, cpp_axis)